...
StitchedSurfaceNets::StitchedSurfaceNets(const JarVoxelTerrain &terrain) : MeshExtractor()
{
...
void StitchedSurfaceNets::create_vertex(const int node_id, const std::vector<int> &neighbours, const bool on_ring)
{
    ...
    for (auto &edge : MeshChunk::Edges)
    {
        ...
        auto colorA = na->get_material_index() == 0 ? glm::vec4(0, 0, 0, 0) : glm::vec4(1, 0, 0, 1);
        auto colorB = nb->get_material_index() == 0 ? glm::vec4(0, 0, 0, 0) : glm::vec4(1, 0, 0, 1);
...
void StitchedSurfaceNets::stitch_inner_faces() {
    for (size_t node_id = 0; node_id < _meshChunk->innerNodeCount; ++node_id) {
...
void StitchedSurfaceNets::stitch_edge_chunk_boundaries() {
...
ChunkMeshData* StitchedSurfaceNets::build_chunk_mesh_data(const JarVoxelTerrain& terrain) const {
    if (_indices.is_empty()) return nullptr;
